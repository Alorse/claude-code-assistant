...
export interface ClaudeMessage {
  type: 'userInput' | 'output' | 'error' | 'loading' | 'clearLoading' | 'setProcessing' | 'toolUse' | 'toolResult' | 'thinking' | 'sessionInfo' | 'updateTokens';
  data?: any;
}
...
export class ClaudeService {
  ...
  private async executeClaudeCommand(message: string, options: ProcessingOptions): Promise<void> {
  ...
  private spawnWSLProcess(args: string[], options: any): cp.ChildProcess {
    ...
    return cp.spawn('wsl', ['-d', wslDistro, 'bash', '-ic', wslCommand], options);
  }

  private spawnNativeProcess(claudeCommand: string, args: string[], options: any): cp.ChildProcess {
