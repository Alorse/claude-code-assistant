...
export class ClaudeAssistantProvider {
	...
	private commits: Array<{ id: string, sha: string, message: string, timestamp: string }> = [];
	...
	private pendingPermissionResolvers: Map<string, (approved: boolean) => void> | undefined;
	private currentConversation: Array<{ timestamp: string, messageType: string, data: any }> = [];
	...
	constructor(
		...
		this.selectedModel = this.context.workspaceState.get('claude.selectedModel', 'default');
	...
	public show(column: vscode.ViewColumn | vscode.Uri = vscode.ViewColumn.Two) {
		...
		this.panel.webview.html = getHtmlForWebview(vscode.env?.isTelemetryEnabled, this.extensionUri.fsPath, this.panel.webview);
	...
	public showInWebview(webview: vscode.Webview, webviewView?: vscode.WebviewView) {
		...
		this.webview.html = getHtmlForWebview(vscode.env?.isTelemetryEnabled, this.extensionUri.fsPath, this.webview);
	...
	private handleWebviewMessage(message: any) {
		switch (message.type) {
			case 'sendMessage':
				this.sendMessageToClaude(message.text, message.planMode, message.thinkingMode);
				return;
			case 'newSession':
				this.newSession();
				return;
	...
	private async sendMessageToClaude(message: string, planMode?: boolean, thinkingMode?: boolean) {
		console.log('Sending message to Claude:', message, { planMode, thinkingMode });
	...
	private sendReadyMessage() {
		this.postMessage({
			type: 'ready',
			data: this.isProcessing ? 'Claude is working...' : 'Ready to chat with Claude Code! Type your message below.'
		});
	...
	private sendPlatformInfo() {
		...
		const dismissed = this.context.globalState.get<boolean>('wslAlertDismissed', false);
	...
	private getLatestConversation(): any | undefined {
		return this.conversationIndex.length > 0 ? this.conversationIndex[0] : undefined;
	}
